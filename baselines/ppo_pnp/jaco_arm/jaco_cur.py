...
class JacoCurEnv():
    def __init__(self,
        ...
        self.episode = 0
        self.expert_obs = np.load('data/pnp_demo.npz')
        self.expert_obs = self.expert_obs['obs']
    ...
    def reset(self):
        ...
        qpos[:9] = self.expert_obs[3][27][:9]
        qvel[:9] = self.expert_obs[3][27][9:18]
    ...
    def step(self, a):
        ...
        if hand_d < 0.10:
            if self.sim.data.get_body_xpos("target")[2] > 0.055 and self.sim.data.get_body_xpos("target")[2] < 0.3:
                reward += (self.sim.data.get_body_xpos("target")[2] - 0.030) * 3000

        if self.sim.data.get_body_xpos("target")[2] > 0.15:
            self.lifted = True
        ...
        if hand_d < 0.12 and self.lifted:
            ...
            reward += 10000 * (1/(np.linalg.norm([0.5, -0.15, 0.03] - self.sim.data.get_body_xpos("target")) + 0.01))
