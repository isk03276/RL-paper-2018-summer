...
class JacoCurEnv():
    def __init__(self,
        ...
        self.episode = 0
        self.expert_obs = np.load('data/lift_demo.npz')
    ...
    def reset(self):
        ...
        qpos[:9] = self.expert_obs[3][25][:9]
        qvel[:9] = self.expert_obs[3][25][9:18]
    ...
    def step(self, a):
        ...
        # So hard
        if hand_d < 0.10:
            if self.sim.data.get_body_xpos("target")[2] > 0.055:
                reward += (self.sim.data.get_body_xpos("target")[2] - 0.030) * 8000


        success = False

        if self.sim.data.get_body_xpos("target")[2] > 0.4:
            success = True
